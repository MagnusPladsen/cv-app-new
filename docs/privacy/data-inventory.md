...
| Supabase `retention_notices` (same region) | User id and the date an inactivity warning was sent | Deleted when the person signs in again, or with the account |
| Vercel server logs | IP address, user agent, request path | One hour on the current plan. No CV content is logged, and neither is any email address |
