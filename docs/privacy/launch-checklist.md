...
| Cookieless analytics, or a compliant consent banner | done | Vercel Web Analytics and Speed Insights: no cookie and no browser storage, so no banner is required under *ekomloven § 3-15*. What it collects is in the policy, and the test fails if that disappears |
...
| No retention rule for inactive accounts (Art. 5(1)(e)) | **Implemented 2026-09-14.** 24 months inactive → emailed warning → 30 days → deleted, cancelled by signing in. `supabase/migrations/20260914000003_retention.sql` and `/api/retention`, daily. Needs the two operator steps in `docs/privacy/retention.md` before it does anything |
